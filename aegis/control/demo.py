...
from aegis.control import capsules, packages, leases
from aegis.control.data import add_source, canaries, IsolatedCache, tripwire
from aegis.control.policy import request_approval, decide, SKILLS
from aegis.control.store import get, put, require, Denied, receipt, digest
from aegis.control.runtime import GovernedRunner


def package_spec(version=3, profile="safe", name="aegis-offline-demo"):
    ...
    manifest = {"name": name, "kind": "model", "version": version, "artifact_hash": hashlib.sha256(artifact.encode()).hexdigest(),
                "signer": "aegis-demo-publisher", "tokenizer_hash": digest("mock-tokenizer-v1"), "adapter_hash": digest("no-adapter"),
