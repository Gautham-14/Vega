...
def worker():
    from vega.storage.database import init_db, execute_write, query_one
    from vega.control.retrieval import verify_claim
    from vega.control.governance import network_call
    ...
    def expired():
        with patch("vega.control.leases.time.time", return_value=time.time() + 1000):
    ...
    def cleanup_failure():
        with patch("vega.control.data.shutil.rmtree", side_effect=OSError("Injected cleanup failure")):
...
def run_self_test():
    from vega import config
    with tempfile.TemporaryDirectory(prefix="vega-adversarial-") as directory:
        env = {**os.environ, "VEGA_DATA_DIR": directory}
        process = subprocess.run([sys.executable, "-m", "vega.control.self_test", "--worker"], cwd=config.BASE_DIR,
