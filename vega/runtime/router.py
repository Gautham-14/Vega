"""
Vega Sovereign AI Runtime - Model Router
Routes tasks to qualified, eligible models based on modality, risk, and hardware headroom.
"""
from typing import Dict, Any, Optional
from vega.models.registry import get_all_models, get_model_by_id, model_integrity_verified, APPROVED_SOVEREIGN_LICENSES
from vega.hardware.scheduler import evaluate_model_eligibility
...
class ModelRouter:
    ...
    def route_task(
        ...
        preferred_id = "VEGA-DEMO-TEXT"
        if task_type in ["vision", "pid", "scanned_doc"]:
            preferred_id = "VEGA-DEMO-VISION"
        elif task_type in ["code", "math", "calculator"]:
            preferred_id = "VEGA-DEMO-CODE"

        capability = required_capability or {
            "VEGA-DEMO-TEXT": "text", "VEGA-DEMO-CODE": "coding",
            "VEGA-DEMO-VISION": "image analysis"
        }[preferred_id]
        ...
        # Fallback to VEGA-DEMO-TEXT if needed
