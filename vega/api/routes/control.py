...
def principal(x_vega_actor: str = Header(default="operator")):
    require_demo_mode()
    return policy.actor(x_vega_actor)["id"]
...
@router.get("/status")
def status():
    return {"enabled": os.environ.get("VEGA_ENABLE_DEMO_ENDPOINTS") == "1", "mode": "Software-simulated attestation",
...
def hardware(package_id: str, identity=Depends(principal)):
    from vega.hardware.simulation import HARDWARE_PROFILES
...
def self_test(identity=Depends(principal)):
    from vega.control.self_test import run_self_test
    return run_self_test()


@router.post("/policies/versions")
def update_versions(req: VersionPolicy, identity=Depends(principal)):
    from vega.control.governance import version_policy
...
def learning(req: LearningPlan, identity=Depends(principal)):
    from vega.control.governance import learning_plan
