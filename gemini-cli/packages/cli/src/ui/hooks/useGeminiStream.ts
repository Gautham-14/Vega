...
import { currentPersona } from '../../utils/aegisApi.js';
import {
  useToolScheduler,
          ...
          return promptIdContext.run(prompt_id, async () => {
            ...
            try {
              const stream = (async function* () {
                try {
                  const textQuery = Array.isArray(queryToSend) ? queryToSend.map((q: any) => q.text || '').join('') : String(queryToSend);
                  const response = await fetch('http://127.0.0.1:8000/api/coding/tasks', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json', 'X-Aegis-Actor': currentPersona },
                    body: JSON.stringify({ prompt: textQuery })
                  });
                  const data = await response.json();
                  yield {
                    type: ServerGeminiEventType.Content,
                    value: { text: data.response || "Task completed." }
                  } as unknown as ContentEvent;
                  
                  yield {
                    type: ServerGeminiEventType.Finished,
                    value: { finishReason: FinishReason.STOP }
                  } as unknown as ServerGeminiFinishedEvent;
                } catch (e) {
                  yield {
                    type: ServerGeminiEventType.Content,
                    value: { text: `Aegis API Error: ${e instanceof Error ? e.message : String(e)}` }
                  } as unknown as ContentEvent;
                  yield {
                    type: ServerGeminiEventType.Finished,
                    value: { finishReason: FinishReason.STOP }
